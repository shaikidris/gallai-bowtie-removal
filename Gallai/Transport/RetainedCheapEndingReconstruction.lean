/-
Copyright (c) 2026 Idris Ali Shaik.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Idris Ali Shaik (with Codex assistance)
-/
import Gallai.Transport.RetainedCheapEndingAccounting
import Gallai.Transport.RetainedEdgePartition
import Mathlib.Logic.Equiv.Fin.Basic
import Mathlib.Data.Fintype.Sum

/-! # One-credit reconstruction in the retained terminal profile -/
namespace Gallai.WholeBowtie
open Certificate Certificate.ThreeAnchor
variable {V : Type*} [Fintype V] [DecidableEq V]
variable {G : SimpleGraph V} [DecidableRel G.Adj] (B : WholeBowtie G)
variable (A : Fin 3 ↪ {v : V // v ∉ B.vertices}) (s : State)
variable (hrows : ∀ i v, v ∈ B.row i ↔
  ∃ j, (rowMask (s i)).val.testBit j.val = true ∧ (A j).val = v)

include hrows in
/-- The two-hub-tail ending certificate reconstructs G within one added path,
preserving all surviving endpoint reserves away from the hub and remote anchor. -/
theorem retainedCheapEnding_reconstruction
    (H : SimpleGraph B.StarSurvivor) [DecidableRel H.Adj] (repair : Bool)
    (hAdj : ∀ u v, H.Adj u v ↔ G.Adj u.val v.val ∧
      ¬ (repair = true ∧ s(u.val,v.val) = s((A 1).val,(A 2).val)))
    (hp : repair = true → G.Adj (A 1).val (A 2).val)
    (hpq : H.Adj B.completedHub (B.retainedSlot A 0) ∧ H.Adj B.completedHub (B.retainedSlot A 1))
    (hr : ¬ H.Reachable B.completedHub (B.retainedSlot A 2))
    (D : Decomposition H) (hd : H.degree B.completedHub = 2)
    (hz : 0 < D.endpointCount B.completedHub) (hpos : 0 < D.endpointCount (B.retainedSlot A 2))
    (u v remote : List Vertex) (added : List (List Vertex))
    (h : Retained.CheapEndingAccepted s repair [u,v] remote added) :
    ∃ E : Decomposition G, E.size = D.size + added.length ∧ E.size ≤ D.size + 1 ∧
      ∀ w : B.StarSurvivor, w ≠ B.completedHub → w ≠ B.retainedSlot A 2 →
        D.endpointCount w ≤ E.endpointCount w.val := by
  classical
  have hrow : ∀ i j, (rowMask (s i)).val.testBit j.val = true → (A j).val ∈ B.row i := by
    intro i j hj
    exact (hrows i _).mpr ⟨j,hj,rfl⟩
  obtain ⟨F,hF,hEnd⟩ := B.retainedCheapEnding_word_family A s hrow H
    (fun u v ha => ((hAdj u v).mp ha).1) hpq hr D hd hz hpos repair hp u v remote added h
  have hsub : (targetEdges s \ {s(0,anchor 0),s(0,anchor 1)}) ∪ retainedRepair repair ⊆
      targetEdges s ∪ retainedRepair repair := by
    intro e he
    rcases Finset.mem_union.mp he with ht | hr
    · exact Finset.mem_union_left _ (Finset.mem_sdiff.mp ht).1
    · exact Finset.mem_union_right _ hr
  have hacc := h.2.2.2.2.2.2.1
  let N (i : Fin added.length) := B.realizeRetainedSubsetMember A s hrow repair hp _ hsub
    ([u,v] ++ remote :: added) hacc added[i] (by simp)
  let Q : Fin D.size ⊕ Fin added.length → NonemptyPath G := Sum.elim F N
  have hN (i : Fin added.length) : (N i).walk.edges =
      (wordEdges added[i]).map (Sym2.map (B.threeLabel A)) := by
    change (NonemptyPath.ofWord _ _ _ _).walk.edges = _
    rw [NonemptyPath.ofWord_edges]
    exact wordEdges_map (B.threeLabel A) added[i]
  have hcover (e : Sym2 V) (he : e ∈ G.edgeSet) : (∑ i, (Q i).walk.edges.count e) = 1 := by
    have hc := hF e
    have hl := B.retainedCheapEnding_local_count A s repair u v remote added h e
    have hb := B.retained_edge_balance A s hrows H repair hAdj D h.1 e he
    rw [Fintype.sum_sum_type]
    simp only [Q,Sum.elim_inl,Sum.elim_inr,hN]
    omega
  let E : Decomposition G := Decomposition.ofEdgeCounts
    (fun i : Fin (D.size + added.length) => Q (finSumFinEquiv.symm i)) (by
      intro e he
      exact ((finSumFinEquiv : Fin D.size ⊕ Fin added.length ≃
        Fin (D.size + added.length)).symm.sum_comp
          (fun i => (Q i).walk.edges.count e)).trans (hcover e he))
  refine ⟨E,rfl,Nat.add_le_add_left h.2.2.2.2.2.2.2 _,?_⟩
  intro w hwx hwr
  change D.endpointCount w ≤ ∑ i : Fin (D.size + added.length),
    ((if (Q (finSumFinEquiv.symm i)).start = w.val then 1 else 0) +
      (if (Q (finSumFinEquiv.symm i)).finish = w.val then 1 else 0))
  rw [(finSumFinEquiv : Fin D.size ⊕ Fin added.length ≃
    Fin (D.size + added.length)).symm.sum_comp
      (fun i => (if (Q i).start = w.val then 1 else 0) +
        (if (Q i).finish = w.val then 1 else 0)),Fintype.sum_sum_type]
  exact (hEnd w hwx hwr).trans (Nat.le_add_right _ _)

end Gallai.WholeBowtie
