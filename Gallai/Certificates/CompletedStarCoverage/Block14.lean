...
import Gallai.Certificates.CompletedStarCoverage.Block14.Sub0
import Gallai.Certificates.CompletedStarCoverage.Block14.Sub1
import Gallai.Certificates.CompletedStarCoverage.Block14.Sub2
import Gallai.Certificates.CompletedStarCoverage.Block14.Sub3
...
private theorem witness_chunked (q : Fin 4) (r : Fin 64) :
    transformedState (action ⟨q.val * 64 + r.val, by omega⟩)
      ...
      (Catalogue.state (orbit ⟨q.val * 64 + r.val, by omega⟩)) =
        encodedAt (blockIndex 14 ⟨q.val * 64 + r.val, by omega⟩) := by
  fin_cases q
