...
-/
import Gallai.Certificates.ZeroSyndromeCoverage.Block13.Core
import Gallai.Certificates.ZeroSyndromeCoverage.Block13.Sub0
import Gallai.Certificates.ZeroSyndromeCoverage.Block13.Sub1
import Gallai.Certificates.ZeroSyndromeCoverage.Block13.Sub2
import Gallai.Certificates.ZeroSyndromeCoverage.Block13.Sub3
...
private theorem witness_chunked (q : Fin 4) (r : Fin 64) :
    HighBoundary (zeroEncodedAt (blockIndex 13 ⟨q.val * 64 + r.val, by omega⟩)) →
    transformedState (action ⟨q.val * 64 + r.val, by omega⟩)
      ...
      (ZeroSyndrome.Catalogue.state (orbit ⟨q.val * 64 + r.val, by omega⟩)) =
        zeroEncodedAt (blockIndex 13 ⟨q.val * 64 + r.val, by omega⟩) := by
  fin_cases q
