...
-/
import Gallai.Certificates.ZeroSyndromeCoverage.Block02.Core
import Gallai.Certificates.ZeroSyndromeCoverage.Block02.Sub0
import Gallai.Certificates.ZeroSyndromeCoverage.Block02.Sub1
import Gallai.Certificates.ZeroSyndromeCoverage.Block02.Sub2
import Gallai.Certificates.ZeroSyndromeCoverage.Block02.Sub3
...
private theorem witness_chunked (q : Fin 4) (r : Fin 64) :
    HighBoundary (zeroEncodedAt (blockIndex 2 ⟨q.val * 64 + r.val, by omega⟩)) →
    transformedState (action ⟨q.val * 64 + r.val, by omega⟩)
      ...
      (ZeroSyndrome.Catalogue.state (orbit ⟨q.val * 64 + r.val, by omega⟩)) =
        zeroEncodedAt (blockIndex 2 ⟨q.val * 64 + r.val, by omega⟩) := by
  fin_cases q
