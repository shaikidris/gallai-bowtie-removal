...
import Gallai.Certificates.SizeTwoCoverage.Block11.Sub0
import Gallai.Certificates.SizeTwoCoverage.Block11.Sub1
import Gallai.Certificates.SizeTwoCoverage.Block11.Sub2
import Gallai.Certificates.SizeTwoCoverage.Block11.Sub3
...
private theorem witness_chunked (q : Fin 4) (r : Fin 64) :
    ...
    transformedState (action ⟨q.val * 64 + r.val, by omega⟩)
      ...
      (SizeTwo.Catalogue.Rows.state (orbit ⟨q.val * 64 + r.val, by omega⟩)) =
        pairEncodedAt (blockIndex 11 ⟨q.val * 64 + r.val, by omega⟩) := by
  fin_cases q
